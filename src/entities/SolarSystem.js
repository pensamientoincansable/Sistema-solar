...
export class SolarSystem {
  ...
  init() {
    ...
    // Intentar textura sun si existe material_baseColor.jpeg
    try {
      const sunTex = this.textureLoader.load('/textures/material_baseColor.jpeg');
    ...
    this.sunLight.castShadow = false; // desactivado para performance, se activa en quality high
    ...
    // Estrellas - instanced para performance
  ...
  update(delta, elapsed) {
    this.sun.rotation.y += 0.0005 * delta * 60;
    this.planets.forEach(p => p.update(delta, elapsed));
    // Parallax sutil estrellas
    if (this.stars) {
      this.stars.rotation.y += 0.00002 * delta * 60;
    }
  }
